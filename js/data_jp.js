    ...
    projects: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            フォントには典雅さと現代性を兼ね備えた Noto Serif TC・Klee One・Zen Maru Gothicを採用し、
      ...
      },
      {
        ...
        items: [
          ...
          {
            label: "コンセプト",
            description: `自作のオリジナルキャラクター「モナクシア」を核として、
            深夜の闇と幽冷な水色の発光感をレイアウトデザインに落とし込みました。
            MYFILEと統一した藍紫系カラーパレットを採用し、
            半透明の背景レイヤー・川流れアニメーション背景・box-shadowの発光効果を組み合わせることで、
            神秘的かつダークな雰囲気を強調しております。
            全体のレイアウトはBootstrapのグリッドシステムを活用した柔軟な構成とし、
            左側にキャラクターイラストの表示エリア、
            右側はタブ切り替えにより自己紹介・好み・DNI などの情報を提示する設計といたしました。
            情報密度と閲覧性の両立を図りつつ、
            Toyhouseの限られたテンプレート環境内において高度にカスタマイズされたビジュアルを実現いたしました。`,
          },
          {
            label: "カラーパレット",
            palette: [
          ...
          },
          {
            ...
            list: [
              {
                text: "ToyhouseプラットフォームにおけるCSSの適用範囲が制限されており、プラットフォーム固有のスタイルを直接上書きできない箇所が存在した",
                solution:
                  "各要素にinline styleを直接記述するか、詳細度の高いセレクタを用いてカスタムスタイルを強制適用した",
                note: "この経験を通じてプラットフォーム環境の制約を深く理解するとともに、Bootstrapクラスとカスタムスタイルを柔軟に組み合わせる技術を習得した",
              },
            ],
          },
          {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            フォントには古典的な趣と現代性を兼ね備えたShippori Mincho B1・LXGW WenKai TC・Noto Serif TCを採用し、
            見出し・ナビゲーション・本文それぞれの役割に応じた階層的な組版を実現し、
            伝統的な茶文化の文字美学を強化しております。
            インタラクションデザインの核にはGSAP ScrollTriggerによるスクロール連動アニメーションを据え、
          ...
          },
          {
            ...
            list: [
              {
                text: "GSAP ScrollTriggerとReactコンポーネントのライフサイクルが競合し、コンポーネントの再レンダリング後にアニメーションが機能しなくなる問題が発生した",
                solution:
                  "GSAPの初期化ロジックをuseEffect内に統一し、gsapSetup.jsにてプラグイン登録を一元管理することで、アニメーションがコンポーネントのマウント後に一度だけ実行されるよう制御した",
                note: "この対応を通じてReactのレンダリング機構と副作用処理のタイミングへの理解を深めるとともに、再利用可能なGSAP初期化モジュールを構築した",
              },
              {
                text: "レスポンシブデザインにおいて、ブレークポイントによってはナビゲーションバーとコンテンツエリアが重複・位置ずれする問題が発生した",
                solution:
                  "CSSカスタムプロパティ（--navHeight）によりナビゲーションバーの高さを一元管理し、calc() を用いて各セクションのmargin-topを動的に算出。加えてmedia queryにて狭い画面向けのレイアウト調整を実施した",
